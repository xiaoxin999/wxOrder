<template>
	<section class='choose_shop'>
		<section class="chooseList" v-for="(item, index) in shopList" @click="checkShop(item,item.id)" :key="index">
			<div>
				<img :src="getUrl(item.logoImage)" alt="">
			</div>
			<ul>
				<li>
					<div class="text-over name_div">{{item.name}}</div> <span class="distance">{{changeKm(item.distance)}}</span>
				</li>
...
<script>
	...
	export default {
		...
		methods: {
			...
			checkShop(item, id) {
				storage.session('nearbyStoreDetail', item);
				this.$router.push({
					path: '/memberRegister',
					query: {
						id
			...
			async getShopList() {
				...
				for (let i = 0; i < this.shopList.length; i++) {
					...
					this.shopList = data.shopList.filter((oftenitem) => {
						return oftenitem.oftenShop == 1;
					});
