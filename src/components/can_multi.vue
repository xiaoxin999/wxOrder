	...
	export default {
		...
		data() {
			return {
				...
				type: true, //选择开始时间
				now: new Date(), // 当前时间
				start: new Date(), //开始的时间（秒）
				end: new Date(), //结束的时间戳（秒）
				nowMonth: null,
				nowYear: null,
				starttime:'',
				endtime:'',
		...
		mounted: function() {
			if(this.sideStart){
                this.start = new Date(this.sideStart);
                this.now = new Date(this.sideStart);
            }
            if(this.sideEnd)this.end = new Date(this.sideEnd);
            this.init();
		},
		methods: {
			changeType: function() {
			...
			init: function() {
				...
				this.index = (new Date(this.now.getFullYear(), this.now.getMonth(), 1)).getDay();
			...
			clickHandle: function(i) {
				...
				console.log(i - 1);
				if(this.type) {
					this.start = (new Date(this.now.getFullYear(), this.now.getMonth(), i - this.index)).setZero();
					console.log(this.start)
				} else {
					this.end = (new Date(this.now.getFullYear(), this.now.getMonth(), i - this.index)).setZero();
			...
			isSelected: function(i, type) {
				var selectedDate = (new Date(this.now.getFullYear(), this.now.getMonth(), i - this.index)).setZero();
			...
			confirm: function() {
				if(this.apartDays) {
					// 计算相隔天数
					var days = this.end.getTime() - this.start.getTime();
					days = (days / (60 * 60 * 24 * 1000)) + 1;
					if(this.apartDays < days) {
						store.commit("setWin",{title:"提示信息:",content:'相隔天数大于' + this.apartDays + '天，请重新选择'})
						return false;
					}
				}

				this.open = false;
				this.starttime = parseFloat(this.start.getTime());
				this.endtime = parseFloat(this.end.getTime());
				this.time = true;
			},
			//选中重置
			back: function() {
