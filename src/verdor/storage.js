...
let storage = {
	 ...
	 */
	webStorageHandle: function(storage, key, value) {
		if (key !== null) {
			...
			if (value !== undefined && value !== null) {
				...
				if (value === null) {
					...
					if (reg.test(fstr)) {
						let ix = fstr.indexOf(':');
						let vstr = fstr.substring(ix + 1);
						let vtype = fstr.substring(0, ix);
						switch (vtype) {
							case '1':
								return vstr === '1';
							case '2':
	...
	async localData(key, value) {
		...
		if (window.indexedDB) {
			...
			if (value === null) {
				await global.db.delete(key);
			} else if (key != undefined && value != undefined) {
				//写入
				await global.db.setData({
					key,
					value
				});
			} else {
				//获取
				let data = await global.db.getData(key);
				return data;
			}

		} else {
			let str = this.webStorageHandle(window.localStorage, key, value);
	...
	countNum: function(key, value) {
		...
		setTimeout(() => {
			...
			let {
				l,
				stack
			} = this.localData('content', undefined, false);
